...
public class StunServer {
	...
	public StunServer(final int primaryPort, final InetAddress primary, final int secondaryPort, final InetAddress secondary)
	...
	public void start() throws SocketException {
		for (final DatagramSocket socket : sockets) {
			...
			final StunServerReceiverThread ssrt = new StunServerReceiverThread(socket);
			ssrt.start();
		}
	}

	/*
	 * Inner class to handle incoming packets and react accordingly. I decided not to start a thread for every received Binding Request,
	 * because the time required to receive a Binding Request, parse it, generate a Binding Response and send it varies only between 2 and 4
	 * milliseconds. This amount of time is small enough so that no extra thread is needed for incoming Binding Request.
	 */
	class StunServerReceiverThread extends Thread {
		...
		StunServerReceiverThread(final DatagramSocket datagramSocket) {
		...
		public void run() {
			while (true) {
				try {
					...
					LOGGER.debug(receiverSocket.getLocalAddress().getHostAddress() + ":" + receiverSocket.getLocalPort()
							+ " datagram received from " + receive.getAddress().getHostAddress() + ":" + receive.getPort());
					final MessageHeader receiveMH = MessageHeader.parseHeader(receive.getData());
					try {
						receiveMH.parseAttributes(receive.getData());
						if (receiveMH.getType() == MessageHeaderType.BindingRequest) {
							LOGGER.debug(receiverSocket.getLocalAddress().getHostAddress() + ":" + receiverSocket.getLocalPort()
									+ " Binding Request received from " + receive.getAddress().getHostAddress() + ":" + receive.getPort());
							final ChangeRequest cr = (ChangeRequest) receiveMH.getMessageAttribute(MessageAttributeType.ChangeRequest);
							if (cr == null) {
							...
							final MessageHeader sendMH = new MessageHeader(MessageHeaderType.BindingResponse);
							...
							if (cr.isChangePort() && !cr.isChangeIP()) {
								...
								LOGGER.debug(changedPort.getLocalAddress().getHostAddress() + ":" + changedPort.getLocalPort()
										+ " send Binding Response to " + send.getAddress().getHostAddress() + ":" + send.getPort());
								...
								changedIP.send(send);
								LOGGER.debug(changedIP.getLocalAddress().getHostAddress() + ":" + changedIP.getLocalPort()
										+ " send Binding Response to " + send.getAddress().getHostAddress() + ":" + send.getPort());
							} else if (!cr.isChangePort() && !cr.isChangeIP()) {
								...
								LOGGER.debug(changedPortIP.getLocalAddress().getHostAddress() + ":" + changedPortIP.getLocalPort()
										+ " send Binding Response to " + send.getAddress().getHostAddress() + ":" + send.getPort());
							}
						}
						...
						final MessageHeader sendMH = new MessageHeader(MessageHeaderType.BindingErrorResponse);
						sendMH.setTransactionID(receiveMH.getTransactionID());
						...
						receiverSocket.send(send);
						LOGGER.debug(changedPortIP.getLocalAddress().getHostAddress() + ":" + changedPortIP.getLocalPort()
								+ " send Binding Error Response to " + send.getAddress().getHostAddress() + ":" + send.getPort());
					}
	...
	public static void main(final String args[]) {
		try {
			...
			final StunServer ss = new StunServer(Integer.parseInt(args[0]), InetAddress.getByName(args[1]),
