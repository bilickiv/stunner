...
public class MessageHeader implements MessageHeaderInterface {
	...
	public MessageHeader(final MessageHeaderType type) {
		super();
		setType(type);
	}

	public void setType(final MessageHeaderType type) {
		this.type = type;
	}
	...
	public static int typeToInteger(final MessageHeaderType type) {
	...
	public void setTransactionID(final byte[] id) {
	...
	public boolean equalTransactionID(final MessageHeader header) {
	...
	public void addMessageAttribute(final MessageAttribute attri) {
		ma.put(attri.getType(), attri);
	}

	public MessageAttribute getMessageAttribute(final MessageAttribute.MessageAttributeType type) {
		return ma.get(type);
	}
	...
	public void parseAttributes(final byte[] data) throws MessageAttributeParsingException {
		try {
			...
			while (length > 0) {
				...
				final MessageAttribute ma = MessageAttribute.parseCommonHeader(cuttedData);
	...
	public static MessageHeader parseHeader(final byte[] data) throws MessageHeaderParsingException {
		try {
